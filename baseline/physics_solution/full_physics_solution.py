...
from __future__ import annotations # 함수 or 클래스가 정의될 때 타입 힌트(어떤 자료형인지?)를 더 유연하게 작성하기 
...
def get_runtime_device() -> torch.device:
    ...
    if hasattr(torch.backends, "mps") and torch.backends.mps.is_available(): # MAC GPU 가속 사용 장치
...
def optimize_runtime_for_device(device: torch.device) -> None:
    if hasattr(torch, "set_float32_matmul_precision"): # 정화도 양보 대신 연산 속도 향상
        try:
...
def extract_motion_targets(data_root: str | Path, out_csv: str | Path, cfg: MotionExtractionConfig) -> pd.DataFrame:
    ...
    for sid, label in sample_iter:
        ...
        total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) # 동영상의 총 프레임 수
        fps = float(cap.get(cv2.CAP_PROP_FPS)) # 초 당 프레임 수
        ok, first = cap.read()
        if not ok:
            cap.release()
            continue
        ...
        if len(mad_to_first) == 0:
            ...
            first_move_thr2 = _first_hit(arr_first, cfg.thr_low) # 처음으로 cfg.thr_low를 넘은 시점 구함
...
class DualViewDataset(Dataset):
    ...
    def _load_img(self, sid: str, view: str) -> tuple[Path, Image.Image]:
        ...
        if view == "top" and self.top_normalizer is not None:
            debug_path = None
            if self.training is True and len(self.top_normalizer._angle_cache) < 20:
                debug_path = Path(f"./debug_plots/train/{sid}_top.png")
            if self.training is False and len(self.top_normalizer._angle_cache) < 20:
                debug_path = Path(f"./debug_plots/dev/{sid}_top.png")
